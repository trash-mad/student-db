...
declare namespace idb {
     ...
     */
    export function openDB<DBTypes extends DBSchema | unknown = unknown>(name: string, version?: number, { blocked, upgrade, blocking, terminated }?: OpenDBCallbacks<DBTypes>): Promise<IDBPDatabase<DBTypes>>;
     ...
     * @template StoreName Names of the object stores to get the types of.
     */
    export type IndexNames<DBTypes extends DBSchema | unknown, StoreName extends StoreNames<DBTypes>> = DBTypes extends DBSchema ? keyof DBTypes[StoreName]['indexes'] : string;
    /**
    ...
    type CursorSource<DBTypes extends DBSchema | unknown, TxStores extends StoreNames<DBTypes>[], StoreName extends StoreNames<DBTypes>, IndexName extends IndexNames<DBTypes, StoreName> | unknown> = IndexName extends IndexNames<DBTypes, StoreName> ? IDBPIndex<DBTypes, TxStores, StoreName, IndexName> : IDBPObjectStore<DBTypes, TxStores, StoreName>;
    ...
    type IDBPCursorWithValueIteratorValueExtends<DBTypes extends DBSchema | unknown = unknown, TxStores extends StoreNames<DBTypes>[] = StoreNames<DBTypes>[], StoreName extends StoreNames<DBTypes> = StoreNames<DBTypes>, IndexName extends IndexNames<DBTypes, StoreName> | unknown = unknown> = Omit<IDBPCursorWithValue<DBTypes, TxStores, StoreName, IndexName>, 'advance' | 'continue' | 'continuePrimaryKey'>;
...
declare namespace idb {
    export const reverseTransformCache: WeakMap<object, any>;
    export function replaceTraps(callback: (currentTraps: ProxyHandler<any>) => ProxyHandler<any>): void;
    /**
...
declare namespace idb {
    interface IDatabaseProviderProps extends OpenDBCallbacks<any> {
        name?: string;
        version?: number;
        children?: React.ReactNode;
    }
    const DatabaseProvider: ({ name, version, children, ...otherProps }: IDatabaseProviderProps) => JSX.Element;
    type useDBHook = <DBTypes extends DBSchema | unknown = unknown>() => IDBPDatabase<DBTypes>;
    const useDB: useDBHook;
}
